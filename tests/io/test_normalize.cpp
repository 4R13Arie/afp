...
TEST(Normalize, TargetHitMinus20dBFS) {
  ...
  auto s = testio::make_sine(1000.f, float(sr), 2048, 10.0f /* intentionally hot */);
  auto wav = testio::write_wav_f32(testio::interleave({s}), 1, sr);
  DecodeParams p; p.normalize_rms = true; p.target_rms_dbfs = -20.f;

  auto dec = new_wav();
  auto outE = dec->decode_bytes(std::span<const std::byte>(reinterpret_cast<const std::byte*>(wav.data()), wav.size()), p);
  ASSERT_TRUE(outE.has_value());
  const auto& y = outE->samples;

  const float rms_db = testio::rms_dbfs(std::span<const float>(y.data(), y.size()));
...
TEST(Normalize, ClampAndNoNaNOnSilence) {
  ...
  const auto& y = outE->samples;
  // Remains silence
  for (float v : y) { ASSERT_FALSE(std::isnan(v)); ASSERT_NEAR(v, 0.0f, 1e-7f); }
}
