...
struct FmtChunk  { char id[4]; uint32_t size; uint16_t audio_fmt; uint16_t ch; uint32_t sr; uint32_t br; uint16_t ba; uint16_t bits; };
struct DataChunk { char id[4]; uint32_t size; };
#pragma pack(pop)
...
inline std::vector<uint8_t> write_wav_f32(const std::vector<float>& interleaved, uint16_t ch, uint32_t sr) {
  ...
  RiffHeader rh{{'R','I','F','F'}, 4 + 8 + static_cast<uint32_t>(sizeof(FmtChunk)) + 8 + data_bytes, {'W','A','V','E'}};
  ...
  std::memcpy(p, &fmt, sizeof(fmt)); p += sizeof(fmt);
  std::memcpy(p, &dc, sizeof(dc)); p += sizeof(dc);
