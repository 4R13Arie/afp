...
inline std::vector<float> make_sine(float freq_hz, float sr, size_t frames,
                                    float amp = 0.5f) {
...
inline std::vector<float> make_silence(size_t frames) {
  return std::vector<float>(frames, 0.0f);
}

inline std::vector<float> make_impulsive(size_t frames, float amp = 1.0f,
                                         size_t every = 16) {
...
interleave(const std::vector<std::vector<float>>& ch) {
  if (ch.empty()) return {};
...
public:
  explicit TempFile(std::string stem = "afp_test") {
    ...
    path_ = (dir / (stem + "-" + std::to_string(::getpid()) + "-" +
                    std::to_string(counter_++) + ".bin")).string();
  }
...
inline std::vector<uint8_t> write_wav_f32(const std::vector<float>& interleaved,
                                          uint16_t ch, uint32_t sr) {
  const uint32_t frames = ch ? (uint32_t)(interleaved.size() / ch) : 0u;
  const uint32_t data_bytes = frames * ch * 4u;
  RiffHeader rh{
      {'R', 'I', 'F', 'F'},
      4 + 8 + static_cast<uint32_t>(sizeof(FmtChunk)) + 8 + data_bytes,
      {'W', 'A', 'V', 'E'}
  };
  FmtChunk fmt{{'f', 'm', 't', ' '}, 16, 3 /*IEEE float*/, ch, sr, sr * ch * 4u,
               (uint16_t)(ch * 4u), 32};
...
inline std::vector<uint8_t> write_wav_s16(const std::vector<float>& interleaved,
                                          uint16_t ch, uint32_t sr) {
  const uint32_t frames = ch ? (uint32_t)(interleaved.size() / ch) : 0u;
  ...
  FmtChunk fmt{{'f', 'm', 't', ' '}, 16, 1 /*PCM*/, ch, sr, sr * ch * 2u,
               (uint16_t)(ch * 2u), 16};
  DataChunk dc{{'d', 'a', 't', 'a'}, data_bytes};
