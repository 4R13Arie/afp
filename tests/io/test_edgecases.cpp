...
TEST(Edge, NonCanonicalWavChunks) {
  ...
  std::array<uint8_t, 12> junkHdr = {'J', 'U', 'N', 'K', 8, 0, 0, 0, 1, 2, 3, 4};
  withjunk.insert(withjunk.begin() + 12 + 24 /*roughly after fmt*/, junkHdr.begin(), junkHdr.end());

  auto fac = afp::io::make_default_decoder_factory();
  auto dec = fac->create_decoder();
  auto out = dec->decode_bytes(
    std::span<const std::byte>(reinterpret_cast<const std::byte *>(withjunk.data()), withjunk.size()), {});
  if (out) {
