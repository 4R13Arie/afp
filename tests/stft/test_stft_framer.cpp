...
TEST(Framer, TailPad_Enabled) {
  ...
  if (expected > 0) {
    ...
    const std::size_t consumed_before_last =
        (std::size_t(expected) - 1) * cfg.hop_size;
    const std::size_t remaining = (N > consumed_before_last)
                                    ? (N - consumed_before_last)
                                    : 0;
...
TEST(Framer, ShortSignal_Cases) {
  ...
  // pad_end=true -> exactly one frame, zero-padded
  {
    ...
    auto outE = fr->segment(teststft::span_of(teststft::make_buffer(sr, x)),
                            cfg);
    ASSERT_TRUE(outE.has_value());
...
TEST(Framer, InvalidArgs) {
  ...
  {
    auto outE = fr->segment(teststft::span_of(buf0),
  ...
  {
    auto outE = fr->segment(teststft::span_of(buf1),
                            FramerConfig{0, 256, true});
    ASSERT_FALSE(outE.has_value());
    EXPECT_EQ(outE.error(), UtilError::InvalidArgument);
  }
  {
    auto outE = fr->segment(teststft::span_of(buf1),
                            FramerConfig{1024, 0, true});
    ASSERT_FALSE(outE.has_value());
    EXPECT_EQ(outE.error(), UtilError::InvalidArgument);
  }
  {
    auto outE = fr->segment(teststft::span_of(buf1),
                            FramerConfig{1024, 2048, true});
    ASSERT_FALSE(outE.has_value());
    EXPECT_EQ(outE.error(), UtilError::InvalidArgument);
  }
}
