...
TEST(Driver, EndToEnd_Sine_BinStable) {
  ...
  const float f = (float(k) * float(sr)) / float(frame);
  // bin-aligned per frame size
  ...
  for (std::uint32_t fidx = 0; fidx < spec.num_frames; ++fidx) {
    ...
    EXPECT_TRUE(
        dom_idx == k || (20.0f * std::log10((dom_val+1e-12f)/(max_other+1e-12f))
          >= 20.0f));
  }
}
...
TEST(Driver, Short_WithPad_YieldsOneFFTFrame) {
  ...
  const std::uint32_t sr = 48000;
  const std::uint32_t frame = 1024, hop = 256;
  auto buf = teststft::make_buffer(sr, teststft::step(800));
  // shorter than frame
...
TEST(Driver, Hop_Affects_FrameCount) {
  ...
  EXPECT_GT(nfB, nfA);
  EXPECT_NEAR(double(nfB) / double(nfA), 4.0, 0.2);
  // allow a slight boundary effect
}
