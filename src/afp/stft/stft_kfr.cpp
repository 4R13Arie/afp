...
inline std::uint32_t compute_num_frames(std::size_t N,
                                        const FramerConfig& cfg) noexcept {
  if (N == 0) return 0;
...
inline void make_hann_window(kfr::univector<float>& w) {
  ...
  for (std::size_t n = 0; n < N; ++n) {
    w[n] = 0.5f - 0.5f * std::cos(
               2.0f * float(M_PI) * (static_cast<float>(n) * invN));
  }
}
} // namespace
...
public:
  Expected<FrameBlock>
  segment(const PcmSpan& in, const FramerConfig& cfg) override {
    ...
    for (std::uint32_t f = 0; f < frames; ++f) {
      ...
      const std::size_t copy_n = std::min<std::size_t>(
          cfg.frame_size, (N > src ? N - src : 0));
      if (copy_n > 0) {
        std::memcpy(fb.data.data() + dst_off, in.samples.data() + src,
                    copy_n * sizeof(float));
      }
      if (copy_n < cfg.frame_size) {
        std::memset(fb.data.data() + dst_off + copy_n, 0,
                    (cfg.frame_size - copy_n) * sizeof(float));
      }
      src += cfg.hop_size;
    }
    return fb;
  ...
  Expected<FrameBlock> apply(const FrameBlock& frames, WindowType wt) override {
    ...
    switch (wt) {
      case WindowType::kHann:
        ...
        break;
      default:
        return tl::unexpected(UtilError::UnsupportedFormat);
    }
...
public:
  ...
  forward_r2c(const FrameBlock& frames, const FftConfig& cfg) override {
    ...
    for (std::uint32_t f = 0; f < frames.num_frames; ++f) {
      const std::size_t off = static_cast<std::size_t>(f) * frames.frame_size;
      std::memcpy(in_v.data(), frames.data.data() + off,
                  sizeof(float) * cfg.fft_size);
      ...
      std::memcpy(out.bins.data() + dst_off, out_v.data(),
                  sizeof(kfr::complex<float>) * out.num_bins);
    }

    return out;
  }

  void warmup(const FftConfig& cfg, std::uint32_t /*max_frames*/) override {
    ensure_plan(cfg.fft_size);
  }
...
public:
  ...
  afp::util::Expected<ComplexSpectra>
  run(const PcmSpan& in, const FramerConfig& fr, WindowType wt,
      const FftConfig& fftc) override {
...
public:
  std::unique_ptr<IFramer> create_framer() override {
    return std::make_unique<FramerKfr>();
  }

  std::unique_ptr<IWindow> create_window() override {
    return std::make_unique<WindowKfr>();
  }

  std::unique_ptr<IFFT> create_fft() override {
    return std::make_unique<FftKfr>();
  }

  std::unique_ptr<IStftDriver> create_driver() override {
    return std::make_unique<StftDriverKfr>();
  }
};
