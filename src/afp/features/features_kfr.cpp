...
inline std::uint32_t hz_to_bin(float hz, afp::util::SampleRateHz sr, std::uint32_t fft_size) noexcept {
  const double binf = (static_cast<double>(hz) * static_cast<double>(fft_size)) /
                      static_cast<double>(sr);
  const double b = std::round(binf); // inclusive band selection: we’ll clamp below
  if (b < 0.0) return 0;
...
inline void whiten_inplace_frame(std::span<float> frame_db, std::uint16_t radius_bins) {
  ...
  if (y.size() > x.size()) y.resize(x.size()); // center/causal mismatch—trim to input length
...
public:
  Expected<Spectrogram> build(const ComplexSpectra& spec_cpx,
    ...
    // Band selection
    const std::uint32_t b0 = std::min(hz_to_bin(params.band_low_hz, params.sample_rate_hz, params.fft_size),
                                      expected_bins - 1);
    const std::uint32_t b1 = std::min(hz_to_bin(params.band_high_hz, params.sample_rate_hz, params.fft_size),
                                      expected_bins - 1);
    ...
    for (std::uint32_t t = 0; t < spec_cpx.num_frames; ++t) {
      const std::size_t src_off = static_cast<std::size_t>(t) * spec_cpx.num_bins;
      ...
      if (params.per_frame_median_subtract) {
        ...
        float med = median_of_span(std::span<float>(scratch.data(), scratch.size()));
      ...
      std::memcpy(out.log_mag.data() + dst_off, dB.data() + b0, out_bins * sizeof(float));
    }
