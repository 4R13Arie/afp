...
inline bool valid_cutoff(float cutoff_hz, SampleRateHz sr) noexcept {
  return (sr > 0) && (cutoff_hz > 0.0f) && (cutoff_hz < 0.5f * static_cast<float>(sr));
}
} // namespace
...
public:
  Expected<PcmBuffer> process(const PcmSpan& in, const BiquadParams& hp) override {
    ...
    try {
      ...
      const double cutoff = std::clamp(static_cast<double>(hp.cutoff_hz), 1.0, nyq * 0.99);
      const auto   norm   = static_cast<float>(cutoff / sr);
      const auto   q      = static_cast<float>(std::max(1e-6, static_cast<double>(hp.Q)));
...
public:
  Expected<PcmBuffer> process(const PcmSpan& in, const BiquadParams& lp) override {
    ...
    try {
      const auto sr     = static_cast<double>(in.sample_rate_hz);
      const double nyq    = std::max(1.0, sr * 0.5);
      const double cutoff = std::clamp(static_cast<double>(lp.cutoff_hz), 1.0, nyq * 0.999);
      const auto   norm   = static_cast<float>(cutoff / sr);
      const auto   q      = static_cast<float>(std::max(1e-6, static_cast<double>(lp.Q)));
...
public:
  Expected<PcmBuffer> process(const PcmSpan& in, const PreEmphasisParams& p) override {
...
public:
  Expected<PcmBuffer> resample(const PcmSpan& in, const ResampleParams& rp) override {
    ...
    try {
      ...
      auto r = kfr::resampler<T>(static_cast<kfr::sample_rate_conversion_quality>(rp.quality),
  ...
  void reset() noexcept override { /* stateless (constructed per call) */ }
};
...
public:
  [[nodiscard]] std::unique_ptr<IHighPass>      create_hpf()         const override { return std::make_unique<HighPassKfr>(); }
  [[nodiscard]] std::unique_ptr<ILowPass>       create_lpf()         const override { return std::make_unique<LowPassKfr>(); }
  [[nodiscard]] std::unique_ptr<IPreEmphasis>   create_preemphasis() const override { return std::make_unique<PreEmphasisKfr>(); }
  [[nodiscard]] std::unique_ptr<IResampler>     create_resampler()   const override { return std::make_unique<ResamplerKfr>(); }
};
