...
void normalize_rms_inplace(kfr::univector<float>& mono, float target_dbfs) {
  const float target_lin = std::pow(10.0f, target_dbfs * 0.05f);
  // 20*log10(s)=dB → s=10^(dB/20)
  const float rms = compute_rms(
...
public:
  [[nodiscard]] Expected<PcmBuffer> to_mono(
      std::span<const PcmSpan> channels) const override {
...
public:
  ...
  Expected<PcmBuffer> decode_file(std::string_view path,
                                  const DecodeParams& params) override {
    drwav wav{};
  ...
  Expected<PcmBuffer> decode_bytes(std::span<const std::byte> data,
                                   const DecodeParams& params) override {
    drwav wav{};
...
private:
  Expected<PcmBuffer> decode_impl(drwav& wav, const DecodeParams& params) {
    ...
    if (total_frames > 0) {
      ...
      drwav_uint64 read = drwav_read_pcm_frames_f32(
      ...
      for (;;) {
        ...
        std::memcpy(interleaved.data() + prev, chunk.data(),
    ...
    for (uint32_t c = 0; c < channels; ++c) {
      spans.push_back(PcmSpan{
          sr, std::span<const float>(ch_data[c].data(), ch_data[c].size())});
    }
...
private:
  Expected<PcmBuffer> decode_impl(drflac& flac, const DecodeParams& params) {
    ...
    drflac_uint64 read = drflac_read_pcm_frames_f32(
        &flac, flac.totalPCMFrameCount, interleaved.data());
    interleaved.resize(static_cast<size_t>(read) * channels);
...
public:
  explicit CompositeDecoder(std::unique_ptr<IDownmixer> dm) {
    ...
    decoders_.push_back(
        std::make_unique<DrWavDecoder>(
            std::make_unique<EnergyPreservingDownmixer>()));
    decoders_.push_back(
        std::make_unique<DrMp3Decoder>(
            std::make_unique<EnergyPreservingDownmixer>()));
    decoders_.push_back(
        std::make_unique<DrFlacDecoder>(
            std::make_unique<EnergyPreservingDownmixer>()));
    downmixer_ = std::move(dm);
    // kept only to adhere to signature symmetry; not used here
  }
...
private:
  Expected<PcmBuffer> try_by_kind(SniffedFormat kind, std::string_view path,
                                  const DecodeParams& params) {
    switch (kind) {
      case SniffedFormat::Wav:
        return decoders_[0]->decode_file(path, params);
      case SniffedFormat::Mp3:
        return decoders_[1]->decode_file(path, params);
      case SniffedFormat::Flac:
        return decoders_[2]->decode_file(path, params);
      default:
        break;
    }
  ...
  Expected<PcmBuffer> try_by_kind(SniffedFormat kind,
                                  std::span<const std::byte> data,
                                  const DecodeParams& params) {
    switch (kind) {
      case SniffedFormat::Wav:
        return decoders_[0]->decode_bytes(data, params);
      case SniffedFormat::Mp3:
        return decoders_[1]->decode_bytes(data, params);
      case SniffedFormat::Flac:
        return decoders_[2]->decode_bytes(data, params);
      default:
        break;
    }
    return tl::unexpected(UtilError::Unavailable);
...
private:
  ...
  std::unique_ptr<IDownmixer> downmixer_;
  // unused in composite but kept for symmetry
};
...
public:
  [[nodiscard]] std::unique_ptr<IAudioDecoder> create_decoder() const override {
    ...
    return std::make_unique<CompositeDecoder>(
        std::make_unique<EnergyPreservingDownmixer>());
  }
};
