...
inline std::uint32_t pack_key(std::uint32_t fa, std::uint32_t ft, std::uint32_t dt_q) noexcept {
  return ((fa & FA_MASK) << FA_SHIFT) | ((ft & FT_MASK) << FT_SHIFT) | (dt_q & DTQ_MASK);
}
...
inline std::int32_t bin_for_offset(std::int64_t offset_ms, std::uint32_t bin_width_ms) noexcept {
  // floor division for possibly negative offsets
  const double v = static_cast<double>(offset_ms) / static_cast<double>(bin_width_ms);
...
public:
        ...
        const MatchParams& mp) const override
  {
    ...
    const std::int32_t dt_tol_codes = static_cast<std::int32_t>(
        std::ceil(static_cast<double>(mp.delta_t_tolerance_ms) / static_cast<double>(DT_STEP_MS)));
    ...
    for (const auto& kv : key_to_qtimes) keys.push_back(FingerprintKey{ kv.first });

    auto lookupsE = index.lookup_batch(std::span<const FingerprintKey>(keys.data(), keys.size()));
    ...
    struct PairHash {
      std::size_t operator()(const std::pair<Track, std::int32_t>& x) const noexcept {
    ...
    for (std::size_t i = 0; i < keys.size(); ++i) {
      ...
      for (const auto& p : postings) {
        for (auto ta : qtimes) {
          const std::int64_t off = static_cast<std::int64_t>(p.t_anchor_ms) - static_cast<std::int64_t>(ta);
    ...
    struct TrackBest { std::int32_t bin; BinStats stats; };
    ...
    for (const auto& kv : bins) {
      ...
      if (it == best.end() || st.count > it->second.stats.count ||
          (st.count == it->second.stats.count && (st.max_q_ms - st.min_q_ms) > (it->second.stats.max_q_ms - it->second.stats.min_q_ms))) {
    ...
    for (const auto& b : best) {
      ...
      const std::uint32_t span = (st.max_q_ms > st.min_q_ms) ? (st.max_q_ms - st.min_q_ms) : 0u;
      if (inliers < mp.min_inliers || span < mp.min_span_ms) continue;

      CandidateMatch cm;
      cm.track = t;
      cm.offset_ms = static_cast<afp::util::TimeMs>(bin * static_cast<std::int32_t>(mp.offset_bin_ms));
      cm.inliers = inliers;
      cm.span_ms = span;
      cm.score = static_cast<float>(inliers) + 0.001f * static_cast<float>(span); // simple composite
      cand.push_back(cm);
    }
