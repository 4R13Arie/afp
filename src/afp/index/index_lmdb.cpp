...
inline UtilError map_mdb(int rc) noexcept {
  ...
  switch (rc) {
    case MDB_NOTFOUND: return UtilError::NotFound;
    case MDB_MAP_FULL: return UtilError::ResourceExhausted;
    case MDB_PANIC:
    case MDB_CORRUPTED: return UtilError::IndexCorrupt;
    case MDB_BAD_VALSIZE:
    case MDB_BAD_DBI:
    case EINVAL: return UtilError::InvalidArgument;
    case ENOSPC:
    case ENOMEM: return UtilError::OutOfMemory;
    default: return UtilError::IOError;
  }
}

// Little-endian put/get
inline void put_u32(std::vector<std::uint8_t>& o, std::uint32_t v) {
  o.push_back((v) & 0xFFu); o.push_back((v >> 8) & 0xFFu); o.push_back((v >> 16) & 0xFFu); o.push_back((v >> 24) & 0xFFu);
}
...
inline std::uint64_t get_u64(const std::uint8_t* p) {
  std::uint64_t v = 0; for (int i = 0; i < 8; ++i) v |= (std::uint64_t)p[i] << (8*i); return v;
}

// Unsigned base-128 varint
inline void put_varu(std::vector<std::uint8_t>& out, std::uint64_t v) {
  while (v >= 0x80) { out.push_back((std::uint8_t)((v & 0x7F) | 0x80)); v >>= 7; }
  out.push_back((std::uint8_t)(v & 0x7F));
}
inline const std::uint8_t* get_varu(const std::uint8_t* p, const std::uint8_t* end, std::uint64_t& v) {
  v = 0; int s = 0;
  while (p < end) {
    std::uint8_t b = *p++;
    v |= (std::uint64_t)(b & 0x7F) << s;
    if (!(b & 0x80)) return p;
    s += 7; if (s > 63) return nullptr;
  }
  return nullptr;
}
...
struct Groups {
  // track -> sorted times
  std::map<TrackId, std::vector<TimeMs>> groups; // ordered to keep stable layout
};

inline void encode_groups(const Groups& g, bool compress, std::vector<std::uint8_t>& out) {
  out.clear();
  out.reserve(128);
  std::uint8_t flags = 0;
...
inline Expected<Groups> decode_groups(const std::uint8_t* data, size_t len) {
  ...
  const std::uint32_t gcnt = get_u32(p); p += 4;
  for (std::uint32_t gi = 0; gi < gcnt; ++gi) {
    if (p + 8 + 4 > end) return tl::unexpected(UtilError::IndexCorrupt);
    const TrackId track = get_u64(p); p += 8;
    const std::uint32_t count = get_u32(p); p += 4;
    ...
    if (compress) {
      ...
      if (p + 4ull * count > end) return tl::unexpected(UtilError::IndexCorrupt);
      for (std::uint32_t i = 0; i < count; ++i) {
        const std::uint32_t t = get_u32(p); p += 4;
        times.push_back(t);
      }
    }
...
inline void merge_times(std::vector<TimeMs>& base, std::span<const TimeMs> add, std::size_t cap_per_key) {
...
public:
  ...
  Expected<void> open(std::string_view path, const BuildOptions& opts) override {
  ...
  Expected<void> insert(TrackId track, FingerprintKey key, TimeMs t_anchor_ms) override {
  ...
  Expected<void> insert_bulk(TrackId track,
    ...
    if (!opened_) return tl::unexpected(UtilError::Unavailable);
    if (keys.size() != t_anchor_ms.size()) return tl::unexpected(UtilError::SizeMismatch);
    auto itK = keys.begin();
  ...
  Expected<void> commit() override {
    ...
    for (auto& kEntry : staging_) {
      ...
      if (rc == 0) {
        auto dec = decode_groups(static_cast<const std::uint8_t*>(v.mv_data), v.mv_size);
        if (!dec) { mdb_txn_abort(txn); return tl::unexpected(dec.error()); }
        g = std::move(*dec);
      } else if (rc != MDB_NOTFOUND) {
        mdb_txn_abort(txn); return tl::unexpected(map_mdb(rc));
      }
      ...
      for (auto& trackEntry : kEntry.second) {
        ...
        times_add.erase(std::unique(times_add.begin(), times_add.end()), times_add.end());
        auto& base = g.groups[trackEntry.first];
        merge_times(base, std::span<const TimeMs>(times_add.data(), times_add.size()),
      ...
      MDB_val nv; nv.mv_size = valbuf.size(); nv.mv_data = valbuf.data();
      rc = mdb_put(txn, dbi_, &k, &nv, 0);
      if (rc != 0) { mdb_txn_abort(txn); return tl::unexpected(map_mdb(rc)); }
    }
...
private:
  // key → (track → times)
  std::unordered_map<std::uint32_t, std::unordered_map<TrackId, std::vector<TimeMs>>> staging_;
...
public:
  ...
  Expected<std::vector<Posting>> lookup(FingerprintKey key) const override {
    if (!opened_.load(std::memory_order_acquire)) return tl::unexpected(UtilError::Unavailable);
    ...
    k.mv_size = sizeof(k32); k.mv_data = &k32;

    rc = mdb_get(txn, dbi_, &k, &v);
    if (rc == MDB_NOTFOUND) { mdb_txn_abort(txn); return std::vector<Posting>{}; }
    if (rc != 0) { mdb_txn_abort(txn); return tl::unexpected(map_mdb(rc)); }
  ...
  lookup_batch(std::span<const FingerprintKey> keys) const override {
    ...
    for (auto kIn : keys) {
      ...
      rc = mdb_get(txn, dbi_, &k, &v);
      if (rc == MDB_NOTFOUND) { out.emplace_back(); continue; }
      if (rc != 0) { mdb_txn_abort(txn); return tl::unexpected(map_mdb(rc)); }
...
private:
  UtilError open_env() {
    ...
    if (rc != 0) { mdb_txn_abort(txn); return map_mdb(rc); }
