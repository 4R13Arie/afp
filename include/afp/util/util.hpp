...
namespace afp::util {
  ...
  struct Spectrogram {
    ...
    std::uint32_t num_frames{};
    kfr::univector<float> log_mag; // row-major [frame][bin], size = num_frames * num_bins
    SampleRateHz sample_rate_hz{};
  ...
  inline std::span<const float> frame_view(const FrameBlock &fb, std::uint32_t i) noexcept {
  ...
  inline std::span<const float> spectrogram_row(const Spectrogram &G, std::uint32_t i) noexcept {
  ...
  inline std::span<float> spectrogram_row(Spectrogram &G, std::uint32_t i) noexcept {
    const std::size_t offset = static_cast<std::size_t>(i) * G.num_bins;
